...
Metreeca/Mark is an minimalist static site generator, optimized for project/app docs. Unlike other solutions it's tightly
...
**less/css** › `.css` / `.less` and `.css` files under the source folder are converted to minified `.css` files at the
same relative path under the target.
...
- open a topic on [groups.google.com/d/forum/metreeca](https://groups.google.com/d/forum/metreeca) to ask how-to or
